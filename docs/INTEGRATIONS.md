...
- `POST /api/portfolio/history` creates a derived snapshot for an explicit goal from bounded normalized import history filters such as source, type, and time window.
...
- Responses contain derived dimensions, clusters, and summaries only; OAuth grants, tokens, authorization codes, private messages, raw activities, and raw provider payloads remain outside this boundary.
- `src/portfolioHistoryUi.js` keeps browser-side route construction, bounded limits, compare summaries, and guarded delete payloads separate from rendering.

These controls advance the personality/feed portfolio model while keeping official imports disabled by default and keeping sensitive inference history reviewable, exportable, comparable, creatable only by explicit request, and deletable.
